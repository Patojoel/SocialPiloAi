...
export const disconnectSocialAccount = createAppAsyncThunk<string, string>(
  'socialAccounts/disconnect',
  async (id, { extra, rejectWithValue }) => {
    try {
      await extra.socialAccountGateway.disconnect(id)
      return id
    } catch (err) {
      ...
      return rejectWithValue({ message: 'Failed to disconnect account' })
    }
  },
)
